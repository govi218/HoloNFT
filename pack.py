...
if len(sys.argv) not in [4, 5]:
    print(f"USAGE: {sys.argv[0]} cover.png content.bin output.png [custom_offset_string]")
    exit()

def get_payload_offset(png_path, custom_offset_str="ENCRYPTED_PAYLOAD"):
    with open(png_path, "rb") as png_in:
        ba = bytearray(png_in.read())
        offset_idx = ba.find(custom_offset_str.encode('utf-8'))
        if offset_idx == -1:
            print("No message detected!")
            return
        png_in.seek(offset_idx + len(custom_offset_str))
        with open("enc_maybe.dat", "wb") as out:
            lines = png_in.readlines()
            lines[-1] = lines[-1][:-16]
...
while True:
    ...
    if chunk_type == b"IEND":
        ...
        if len(custom_offset_str) > 0:
            idat_body += custom_offset_str.encode('utf-8')

        print("idat body before pack: " + str(len(idat_body)))
        # concatenate our content that we want to embed
        idat_body += content_in.read()

        # if len(idat_body) > width * height:
            # exit("ERROR: Input files too big (" + str(len(idat_body)) + ") for cover image resolution.")
